...
func TestBTreeApplyByLevel(t *testing.T) {
	...
	for _, table := range tables {
		root := &TreeNode{Data: table.tree[0]}

		for _, data := range table.tree[1:] {
			BTreeInsertData(root, data)
		}

		res := CaptureOutput(func() {
			BTreeApplyByLevel(root, table.f)
		})
...
func TestBTreeDeleteNode(t *testing.T) {
	...
	for _, table := range tables {
		...
		node := BTreeSearchItem(root, table.delete)
		root = BTreeDeleteNode(root, node)

		res := CaptureOutput(func() {
			BTreeApplyInorder(root, table.f)
		})
...
func TestBTreeTransplant(t *testing.T) {
	...
	for _, table := range tables {
		...
		node := BTreeSearchItem(root, table.search)

		replacement := &TreeNode{Data: table.replacment[0]}

		for _, replData := range table.replacment[1:] {
			BTreeInsertData(replacement, replData)
		}
